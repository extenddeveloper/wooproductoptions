namespace WooOptionsFic.Builder {
  ...
  export function BuilderPage(props: { uuid: string; navigate: (route: string) => void }): any {
    ...
    const addField = (field: WooOptionsFic.FieldDefinition, index?: number, parentUuid?: string) => { actions.addField(field, index, parentUuid); actions.selectField(field.uuid); actions.setInspectorTab('content'); };
    ...
    return <div className="wof-builder">
      ...
      <div className="wof-builder-workspace"><ElementsPanel onAdd={addField} onOpenStyle={() => { actions.selectField(null); actions.setInspectorTab('style'); }} /><Canvas document={state.document} selectedUuid={state.selectedUuid} device={state.device} onSelect={(uuid) => { actions.selectField(uuid); actions.setInspectorTab('content'); }} onAdd={addField} onAddChild={(parentUuid, field, index) => addField(field, index, parentUuid)} onMove={actions.moveField} onMoveChild={actions.moveChildField} onMoveToParent={actions.moveFieldToParent} onDuplicate={(field) => addField(WooOptionsFic.FieldFactory.duplicate(field))} onDelete={setDeleteUuid} /><Inspector field={selectedField} document={state.document} tab={state.inspectorTab} onTabChange={actions.setInspectorTab} onFieldChange={(field) => actions.replaceField(field.uuid, field)} onDocumentChange={actions.updateDocument} onDuplicate={duplicateSelected} onDelete={() => selectedField && setDeleteUuid(selectedField.uuid)} /></div>
