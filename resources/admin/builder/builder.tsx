namespace WooOptionsFic.Builder {
  ...
  const statusLabels: Record<WooOptionsFic.SaveStatus, string> = {
  ...
  export function BuilderPage(props: { uuid: string; navigate: (route: string) => void }): any {
    ...
    const saveNow = useCallback(async (note = 'Builder autosave'): Promise<WooOptionsFic.OptionSetRecord> => {
    ...
    useEffect(() => {
      if (!state.dirty || !state.document || !state.optionSet) return;
